...
export async function buildAuthUrl({ state, nonce, codeChallenge }) {
  ...
  u.searchParams.set('scope', process.env.OIDC_SCOPES || 'openid profile email offline_access')
...
/** Refresh tokens using a refresh_token (requires offline_access on the OIDC app). */
export async function refreshOidcTokens({ refreshToken }) {
  ...
  const data = await res.json().catch(() => ({}))
  if (!res.ok) throw new Error(`OIDC refresh failed ${res.status}: ${JSON.stringify(data)}`)
  if (!data.id_token) throw new Error('OIDC refresh response missing id_token')
  return data
}
